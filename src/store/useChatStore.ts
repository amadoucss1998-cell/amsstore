...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  getMessages: (matchId: string) => get().messages[matchId] ?? [],
  ...
  sendMessage: (matchId: string, text: string, senderId: string) => {
    ...
    set((s) => ({
      messages: { ...s.messages, [matchId]: [...(s.messages[matchId] ?? []), newMsg] },
      matches: s.matches.map((m) => m.id === matchId ? { ...m, lastMessage: newMsg } : m),
    }));
  ...
  simulateReply: (matchId: string, userId: string) => {
    ...
    setTimeout(() => set((s) => ({ isTyping: { ...s.isTyping, [matchId]: true } })), typingDelay);
    setTimeout(() => {
      const text = MOCK_REPLIES[Math.floor(Math.random() * MOCK_REPLIES.length)];
      const reply: Message = { id: generateId(), matchId, senderId: userId, receiverId: 'current_user', text, createdAt: new Date(), read: false, type: 'text' };
      set((s) => ({
        isTyping: { ...s.isTyping, [matchId]: false },
        messages: { ...s.messages, [matchId]: [...(s.messages[matchId] ?? []), reply] },
        matches: s.matches.map((m) => m.id === matchId ? { ...m, lastMessage: reply, unreadCount: m.unreadCount + 1 } : m),
      }));
    }, replyDelay);
  },

  markAsRead: (matchId: string) => set((s) => ({
  ...
  getTotalUnread: () => get().matches.reduce((sum, m) => sum + m.unreadCount, 0),
}));
