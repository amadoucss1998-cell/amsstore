...
export const useSwipeStore = create<SwipeState>((set, get) => ({
  ...
  swipeRight: (user: User) => {
    ...
    if (Math.random() < 0.4) {
      newMatch = { id: generateId(), userId: 'current_user', matchedUserId: user.id, user, createdAt: new Date(), unreadCount: 0 };
    }
    set((s) => ({
  ...
  swipeLeft: (user: User) => {
    ...
    set((s) => {
      ...
      return { currentIndex: s.currentIndex + 1, swipeHistory: [...s.swipeHistory, action], dislikedUserIds: newDisliked };
    });
  },
  ...
  superLike: (user: User) => {
    ...
    const newMatch: Match = { id: generateId(), userId: 'current_user', matchedUserId: user.id, user, createdAt: new Date(), unreadCount: 0 };
    set((s) => ({
      currentIndex: s.currentIndex + 1,
      swipeHistory: [...s.swipeHistory, action],
  ...
  undoLastSwipe: () => {
    set((s) => {
      ...
      return { currentIndex: s.currentIndex - 1, swipeHistory: newHistory, likedUserIds: newLiked, dislikedUserIds: newDisliked, superlikedUserIds: newSuperliked };
    });
  },

  clearNewMatch: () => set({ newMatch: null }),

  loadMoreProfiles: () => set((s) => ({ profiles: [...s.profiles, ...MOCK_PROFILES.map((p) => ({ ...p, id: generateId() }))] })),
}));
