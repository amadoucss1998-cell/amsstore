    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.sliderRow}>
              {[5, 10, 25, 50, 100].map((d) => (
                <TouchableOpacity key={d} style={[styles.sliderChip, maxDist === d && styles.sliderChipActive]} onPress={() => { setMaxDist(d); updatePreferences({ maxDistance: d }); }}>
                  <Text style={[styles.sliderChipText, maxDist === d && styles.sliderChipTextActive]}>{d}mi</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={styles.ageRow}>
              <View style={styles.ageControl}>
                ...
                <View style={styles.ageButtons}>
                  <TouchableOpacity style={styles.ageBtn} onPress={() => setMinAge((v) => Math.max(18, v - 1))}><Text style={styles.ageBtnText}>-</Text></TouchableOpacity>
                  <Text style={styles.ageValue}>{minAge}</Text>
                  <TouchableOpacity style={styles.ageBtn} onPress={() => setMinAge((v) => Math.min(maxAge - 1, v + 1))}><Text style={styles.ageBtnText}>+</Text></TouchableOpacity>
                </View>
              </View>
              ...
              <View style={styles.ageControl}>
                ...
                <View style={styles.ageButtons}>
                  <TouchableOpacity style={styles.ageBtn} onPress={() => setMaxAge((v) => Math.max(minAge + 1, v - 1))}><Text style={styles.ageBtnText}>-</Text></TouchableOpacity>
                  <Text style={styles.ageValue}>{maxAge}</Text>
                  <TouchableOpacity style={styles.ageBtn} onPress={() => setMaxAge((v) => Math.min(65, v + 1))}><Text style={styles.ageBtnText}>+</Text></TouchableOpacity>
                </View>
              </View>
            </View>
          </View>
          ...
          <View style={styles.card}>
            {[
              ...
              <View key={item.label} style={[styles.toggleRow, i > 0 && styles.rowBorder]}>
                ...
                <Switch value={item.value} onValueChange={item.onChange} trackColor={{ false: Colors.border, true: Colors.primary }} thumbColor={Colors.white} />
              </View>
            ))}
          </View>
        </View>
          ...
          <View key={section.title} style={styles.section}>
            ...
            <View style={styles.card}>
              {section.items.map((item, i) => (
                <TouchableOpacity key={item.label} style={[styles.settingsRow, i > 0 && styles.rowBorder]} onPress={item.action} activeOpacity={0.7}>
                  <Text style={styles.settingsRowIcon}>{item.icon}</Text>
                  <Text style={[styles.settingsRowLabel, (item as any).highlight && styles.highlightLabel]}>{item.label}</Text>
        ...
        <View style={styles.logoutSection}>
          <GradientButton title="Sign Out" onPress={logout} variant="outline" size="lg" />
          <TouchableOpacity style={styles.deleteAccount}><Text style={styles.deleteAccountText}>Delete Account</Text></TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  profileCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.cardBackground, borderRadius: BorderRadius.xl, padding: Spacing['4'], marginBottom: Spacing['6'], gap: 14, borderWidth: 1, borderColor: Colors.border },
  profileInfo: { flex: 1, gap: 2 },
  profileName: { fontSize: 18, fontWeight: '700', color: Colors.white },
  profileEmail: { fontSize: 13, color: Colors.textMuted },
  section: { marginBottom: Spacing['6'] },
  sectionTitle: { fontSize: 12, fontWeight: '700', color: Colors.textMuted, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: Spacing['3'] },
  card: { backgroundColor: Colors.cardBackground, borderRadius: BorderRadius.xl, borderWidth: 1, borderColor: Colors.border, overflow: 'hidden' },
  ...
  sliderChip: { paddingVertical: 7, paddingHorizontal: 14, borderRadius: BorderRadius.full, backgroundColor: Colors.background, borderWidth: 1, borderColor: Colors.border },
  ...
  ageBtn: { width: 34, height: 34, borderRadius: 17, backgroundColor: Colors.background, borderWidth: 1, borderColor: Colors.border, alignItems: 'center', justifyContent: 'center' },
