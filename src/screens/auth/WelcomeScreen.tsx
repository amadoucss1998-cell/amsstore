...
export const WelcomeScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const floatAnims = useRef(FLOATING_PROFILES.map(() => new Animated.Value(0))).current;
  ...
  useEffect(() => {
    ...
    floatAnims.forEach((anim, i) => {
      Animated.loop(
        Animated.sequence([
          Animated.delay(FLOATING_PROFILES[i].delay),
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Floating emoji profiles */}
      {FLOATING_PROFILES.map((profile, i) => (
        <Animated.View
          key={i}
          style={[
            styles.floatingProfile,
            {
              top: profile.top,
              left: profile.left,
              width: profile.size,
        ...
        >
          <Text style={{ fontSize: profile.size * 0.55 }}>{profile.emoji}</Text>
        </Animated.View>
      ))}
      ...
      <LinearGradient
        ...
        locations={[0, 0.35, 1]}
        style={[StyleSheet.absoluteFill, { top: H * 0.25 }]}
      />
      ...
      <View style={styles.content}>
        ...
        >
          ...
          <Text style={styles.subtitle}>
            Meet real people, build real connections.{`\n`}Your next story starts here.
          </Text>
        </Animated.View>
