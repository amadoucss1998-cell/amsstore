...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, ScrollView, Dimensions, Animated, Image } from 'react-native';
...
export const OnboardingScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const goToStep = (next: number) => {
    Animated.timing(slideAnim, { toValue: -W, duration: 250, useNativeDriver: true }).start(() => {
      setStep(next); slideAnim.setValue(W);
      Animated.spring(slideAnim, { toValue: 0, useNativeDriver: true, tension: 60, friction: 10 }).start();
    });
  };

  const toggleInterest = (interest: string) => setSelectedInterests((prev) => prev.includes(interest) ? prev.filter((i) => i !== interest) : [...prev, interest].slice(0, 10));

  const handleFinish = () => {
    updateUser({ bio, interests: selectedInterests, photos: selectedPhotos.length > 0 ? selectedPhotos.map((i) => PLACEHOLDER_PHOTOS[i]) : [PLACEHOLDER_PHOTOS[0]] });
    completeOnboarding();
  };
    ...
    switch (step) {
        ...
        <View style={styles.stepContent}>
          ...
          <View style={styles.photoGrid}>
            {PLACEHOLDER_PHOTOS.map((uri, i) => (
              <TouchableOpacity key={i} style={[styles.photoSlot, selectedPhotos.includes(i) && styles.photoSlotSelected]} onPress={() => setSelectedPhotos((prev) => prev.includes(i) ? prev.filter((p) => p !== i) : [...prev, i].slice(0, 6))}>
                <Image source={{ uri }} style={styles.photoThumb} />
                {selectedPhotos.includes(i) && <View style={styles.photoCheckOverlay}><Text style={styles.photoCheck}>✓</Text></View>}
              </TouchableOpacity>
            ))}
            {[...Array(3)].map((_, i) => <TouchableOpacity key={`empty-${i}`} style={[styles.photoSlot, styles.photoSlotEmpty]}><Text style={styles.photoAddIcon}>+</Text></TouchableOpacity>)}
          </View>
        </View>
      );
      case 1: return (
        <View style={styles.stepContent}>
          ...
          <TextInput style={styles.bioInput} value={bio} onChangeText={setBio} multiline numberOfLines={5} placeholder="I love spontaneous road trips, making pasta from scratch, and finding hidden rooftop bars..." placeholderTextColor={Colors.textMuted} maxLength={300} />
        ...
        <View style={styles.stepContent}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} style={styles.interestsScroll}>
            <View style={styles.interestsGrid}>
              {INTERESTS.map((interest) => <Badge key={interest} label={interest} selected={selectedInterests.includes(interest)} onPress={() => toggleInterest(interest)} />)}
            </View>
          </ScrollView>
        </View>
      );
        ...
        <View style={styles.stepContent}>
          ...
          <View style={styles.preferenceSection}>
            ...
            <View style={styles.rangeRow}>
              <TouchableOpacity style={styles.rangeButton} onPress={() => setMinAge((v) => Math.max(18, v - 1))}><Text style={styles.rangeButtonText}>-</Text></TouchableOpacity>
              <Text style={styles.rangeValue}>{minAge} – {maxAge}</Text>
              <TouchableOpacity style={styles.rangeButton} onPress={() => setMaxAge((v) => Math.min(65, v + 1))}><Text style={styles.rangeButtonText}>+</Text></TouchableOpacity>
            </View>
          </View>
          <View style={styles.preferenceSection}>
            ...
            <View style={styles.distanceRow}>
              {[5, 10, 25, 50, 100].map((d) => (
                <TouchableOpacity key={d} style={[styles.distanceChip, maxDistance === d && styles.distanceChipActive]} onPress={() => setMaxDistance(d)}>
                  <Text style={[styles.distanceChipText, maxDistance === d && styles.distanceChipTextActive]}>{d}mi</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
      ...
      case 4: return (
        <View style={[styles.stepContent, styles.finalStep]}>
          <LinearGradient colors={[Colors.gradientStart, Colors.gradientEnd]} style={styles.finalEmoji} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
            <Text style={{ fontSize: 48 }}>⚡</Text>
          </LinearGradient>
          <Text style={styles.stepTitle}>You're all set!</Text>
          <Text style={styles.stepSubtitle}>SparkMatch is ready to find your people.{`\n`}Let the sparks fly.</Text>
          <View style={styles.readyStats}>
            <View style={styles.readyStat}><Text style={styles.readyStatNum}>15+</Text><Text style={styles.readyStatLabel}>Profiles nearby</Text></View>
            <View style={styles.readyDivider} />
            <View style={styles.readyStat}><Text style={styles.readyStatNum}>{selectedInterests.length}</Text><Text style={styles.readyStatLabel}>Interests set</Text></View>
            <View style={styles.readyDivider} />
            <View style={styles.readyStat}><Text style={styles.readyStatNum}>{maxDistance}mi</Text><Text style={styles.readyStatLabel}>Distance range</Text></View>
          </View>
        </View>
      );
      default: return null;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.footer}>
        <GradientButton title={step < TOTAL_STEPS - 1 ? 'Continue' : 'Start Matching ⚡'} onPress={() => { if (step < TOTAL_STEPS - 1) goToStep(step + 1); else handleFinish(); }} size="lg" />
        {step > 0 && step < TOTAL_STEPS - 1 && <TouchableOpacity style={styles.skipStep} onPress={() => goToStep(step + 1)}><Text style={styles.skipStepText}>Skip for now</Text></TouchableOpacity>}
      </View>
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  progressContainer: { paddingHorizontal: Spacing['6'], paddingTop: Spacing['4'], paddingBottom: Spacing['2'], flexDirection: 'row', alignItems: 'center', gap: 12 },
  ...
  photoSlot: { width: (W - 64) / 3, height: (W - 64) / 3, borderRadius: BorderRadius.lg, overflow: 'hidden', backgroundColor: Colors.cardBackground, borderWidth: 2, borderColor: Colors.border },
  photoSlotSelected: { borderColor: Colors.primary },
  photoSlotEmpty: { alignItems: 'center', justifyContent: 'center' },
  photoThumb: { width: '100%', height: '100%' },
  photoCheckOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(255,75,110,0.4)', alignItems: 'center', justifyContent: 'center' },
  photoCheck: { fontSize: 28, color: Colors.white, fontWeight: '800' },
  photoAddIcon: { fontSize: 28, color: Colors.textMuted },
  bioInput: { backgroundColor: Colors.cardBackground, borderWidth: 1, borderColor: Colors.border, borderRadius: BorderRadius.lg, padding: Spacing['4'], fontSize: 15, color: Colors.textPrimary, textAlignVertical: 'top', minHeight: 140, lineHeight: 22 },
  ...
  rangeButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: Colors.cardBackground, borderWidth: 1, borderColor: Colors.border, alignItems: 'center', justifyContent: 'center' },
  ...
  distanceChip: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: BorderRadius.full, backgroundColor: Colors.cardBackground, borderWidth: 1, borderColor: Colors.border },
