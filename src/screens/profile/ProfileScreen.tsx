      ...
      <Animated.View style={[styles.stickyHeader, { opacity: headerOpacity }]}>
        <SafeAreaView edges={['top']}>
          <View style={styles.stickyHeaderContent}>
            ...
            <TouchableOpacity style={styles.settingsButton} onPress={() => navigation.navigate('Settings')}><Text style={styles.settingsIcon}>⚙️</Text></TouchableOpacity>
          </View>
        </SafeAreaView>
      </Animated.View>
      <Animated.ScrollView showsVerticalScrollIndicator={false} onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: scrollY } } }], { useNativeDriver: false })} scrollEventThrottle={16}>
        <SafeAreaView edges={['top']}>
          <View style={styles.photoSection}>
            ...
            <LinearGradient colors={['transparent', Colors.background]} style={styles.photoGradient} />
            <View style={styles.photoActions}>
              <TouchableOpacity style={styles.editPhotoButton}><Text style={styles.editPhotoText}>📷 Edit Photos</Text></TouchableOpacity>
              <TouchableOpacity style={styles.settingsButtonFloat} onPress={() => navigation.navigate('Settings')}><Text style={styles.settingsIcon}>⚙️</Text></TouchableOpacity>
            </View>
          ...
          <View style={styles.completionCard}>
            ...
            <View style={styles.completionBar}>
              <LinearGradient colors={[Colors.gradientStart, Colors.gradientEnd]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[styles.completionFill, { width: `${completionPct}%` }]} />
            </View>
            <View style={styles.completionItems}>
              {completionFields.map((field) => (
                <View key={field.label} style={styles.completionItem}>
                  <Text>{field.done ? '✅' : '○'}</Text>
          ...
          <View style={styles.content}>
            <View style={styles.section}>
              ...
              <Text style={styles.bioText}>{user.bio || 'Add a bio to help people get to know you better...'}</Text>
            </View>
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
              ...
              {user.interests.length > 0 ? (
                ...
                <TouchableOpacity style={styles.addInterestsButton}><Text style={styles.addInterestsText}>+ Add interests</Text></TouchableOpacity>
              )}
            </View>
            <View style={styles.statsCard}>
              <View style={styles.statItem}><Text style={styles.statNumber}>142</Text><Text style={styles.statLabel}>Profile views</Text></View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}><Text style={styles.statNumber}>28</Text><Text style={styles.statLabel}>Likes received</Text></View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}><Text style={styles.statNumber}>6</Text><Text style={styles.statLabel}>Matches</Text></View>
            </View>
          </View>
        </SafeAreaView>
      </Animated.ScrollView>
...
const styles = StyleSheet.create({
  ...
  stickyHeader: { position: 'absolute', top: 0, left: 0, right: 0, zIndex: 100, backgroundColor: Colors.background, borderBottomWidth: 1, borderBottomColor: Colors.border },
  stickyHeaderContent: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: Spacing['5'], paddingVertical: Spacing['3'] },
  ...
  photoActions: { position: 'absolute', top: 50, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: Spacing['4'] },
  editPhotoButton: { backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: BorderRadius.full, paddingVertical: 8, paddingHorizontal: 14 },
  editPhotoText: { color: Colors.white, fontSize: 13, fontWeight: '600' },
  settingsButtonFloat: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
  settingsButton: { width: 36, height: 36, borderRadius: 18, backgroundColor: Colors.cardBackground, alignItems: 'center', justifyContent: 'center' },
  ...
  completionCard: { margin: Spacing['5'], backgroundColor: Colors.cardBackground, borderRadius: BorderRadius.xl, padding: Spacing['4'], borderWidth: 1, borderColor: Colors.border },
  ...
  completionBar: { height: 4, backgroundColor: Colors.border, borderRadius: 2, overflow: 'hidden', marginBottom: 12 },
  ...
  addInterestsButton: { borderWidth: 1, borderColor: Colors.border, borderRadius: BorderRadius.lg, paddingVertical: 10, paddingHorizontal: 16, alignSelf: 'flex-start' },
  addInterestsText: { color: Colors.textMuted, fontSize: 14 },
  statsCard: { backgroundColor: Colors.cardBackground, borderRadius: BorderRadius.xl, padding: Spacing['5'], flexDirection: 'row', borderWidth: 1, borderColor: Colors.border },
