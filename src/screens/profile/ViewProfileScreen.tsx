...
type Props = {
  navigation: NativeStackNavigationProp<SwipeStackParamList, 'ViewProfile'>;
  route: RouteProp<SwipeStackParamList, 'ViewProfile'>;
};
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.photosContainer}>
          ...
          <LinearGradient
            colors={['transparent', 'rgba(15,15,26,0.5)', Colors.background]}
          ...
          <View style={styles.photoDots}>
            {user.photos.map((_, i) => (
              <TouchableOpacity
                key={i}
                style={[styles.dot, i === currentPhoto && styles.dotActive]}
                onPress={() => setCurrentPhoto(i)}
              />
            ))}
          </View>
          ...
          <TouchableOpacity
            ...
            onPress={() => setCurrentPhoto((p) => Math.max(0, p - 1))}
          />
          <TouchableOpacity
            style={styles.tapRight}
            onPress={() => setCurrentPhoto((p) => Math.min(user.photos.length - 1, p + 1))}
          />
            ...
            <View style={styles.quickInfo}>
              {user.height && (
                <View style={styles.infoChip}>
                  ...
                  <Text style={styles.infoChipText}>{user.height}</Text>
                </View>
              )}
              {user.education && (
                <View style={styles.infoChip}>
                  <Text style={styles.infoChipEmoji}>🎓</Text>
                  <Text style={styles.infoChipText}>{user.education}</Text>
                </View>
              )}
              <View style={styles.infoChip}>
                <Text style={styles.infoChipEmoji}>💝</Text>
                <Text style={styles.infoChipText}>{user.lookingFor}</Text>
              </View>
              <View style={styles.infoChip}>
                <Text style={styles.infoChipEmoji}>📍</Text>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.photosGrid}>
                {user.photos.slice(1).map((uri, i) => (
                  <TouchableOpacity key={i} onPress={() => setCurrentPhoto(i + 1)}>
...
const styles = StyleSheet.create({
  ...
  gridPhoto: { width: (W - 60) / 2, height: (W - 60) / 2, borderRadius: BorderRadius.lg },
