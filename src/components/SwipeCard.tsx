...
export const SwipeCard: React.FC<SwipeCardProps> = ({
  ...
  const superLikeOpacity = position.y.interpolate({
    inputRange: [SWIPE_UP_THRESHOLD, SWIPE_UP_THRESHOLD * 0.4],
    outputRange: [1, 0],
    extrapolate: 'clamp',
  });
  ...
  return (
    <Animated.View style={cardStyle} {...(isTop ? panResponder.panHandlers : {})}>
      <TouchableOpacity
        activeOpacity={0.98}
        onPress={() => isTop && onPress(user)}
        style={{ flex: 1 }}
      >
        {/* Photo */}
        <ProfilePhoto
          uri={user.photos[photoIndex]}
          width={W - 32}
          height={H * 0.65}
          showGradient
        />
        ...
        {user.photos.length > 1 && (
          <>
            ...
            <TouchableOpacity
              ...
              onPress={() => setPhotoIndex((p) => Math.min(user.photos.length - 1, p + 1))}
            />
          </>
        )}
