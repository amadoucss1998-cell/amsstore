...
interface ProgressBarProps { current: number; total: number; height?: number; }

export const ProgressBar: React.FC<ProgressBarProps> = ({ current, total, height = 4 }) => {
  const progress = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    Animated.spring(progress, { toValue: (current / total) * 100, useNativeDriver: false, tension: 60, friction: 10 }).start();
  }, [current, total]);
...
const styles = StyleSheet.create({
  track: { width: '100%', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 100, overflow: 'hidden' },
});
